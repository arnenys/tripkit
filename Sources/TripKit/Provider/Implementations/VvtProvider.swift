...
public class VvtProvider: AbstractHafasClientInterfaceProvider {
    ...
    override func newLine(id: String?, network: String?, product: Product?, name: String?, shortName: String?, number: String?, vehicleNumber: String?, networkId: String? = nil) -> Line {
        if product == .tram && name == "HBB" {
            return Line(id: id, network: network, product: .cablecar, label: name, name: "Hungerburgbahn", number: number, vehicleNumber: vehicleNumber, style: lineStyle(network: network, product: .cablecar, label: name), attr: [], message: nil, networkId: networkId)
        } else {
            return super.newLine(id: id, network: network, product: product, name: name, shortName: shortName, number: number, vehicleNumber: vehicleNumber, networkId: networkId)
        }
    }
