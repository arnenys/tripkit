...
public class VbnProvider: AbstractHafasClientInterfaceProvider {
    ...
    override func newLine(id: String?, network: String?, product: Product?, name: String?, shortName: String?, number: String?, vehicleNumber: String?, networkId: String? = nil) -> Line {
        let line = super.newLine(id: id, network: network, product: product, name: name, shortName: shortName, number: number, vehicleNumber: vehicleNumber, networkId: networkId)

        if line.product == .bus && "57" == line.label {
            return Line(id: id, network: line.network, product: line.product, label: line.label, name: line.name, style: line.style, attr: [.serviceReplacement, .circleClockwise], message: line.message, networkId: line.networkId)
        } else if line.product == .bus && "82" == line.label {
            return Line(id: id, network: line.network, product: line.product, label: line.label, name: line.name, style: line.style, attr: [.serviceReplacement, .circleClockwise], message: line.message, networkId: line.networkId)
        } else if line.product == .bus && "58" == line.label {
            return Line(id: id, network: line.network, product: line.product, label: line.label, name: line.name, style: line.style, attr: [.serviceReplacement, .circleAnticlockwise], message: line.message, networkId: line.networkId)
        } else {
            return line
        }
    }
